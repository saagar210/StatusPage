...
if ! docker info >/dev/null 2>&1; then
  echo "Docker is required for authenticated E2E. Start Docker or Colima first."
  exit 1
fi
...
API_LOG="${ROOT_DIR}/.codex/api-e2e.log"
cleanup() {
  if [[ -n "${API_PID:-}" ]]; then
...
echo "Resetting local Postgres and Redis for deterministic E2E..."
docker compose -f docker/docker-compose.dev.yml down -v >/dev/null 2>&1 || true
...
if ! kill -0 "$API_PID" >/dev/null 2>&1; then
  echo "API server exited after health check. Recent log output:"
  tail -n 200 "$API_LOG" || true
  exit 1
fi
...
echo "Running authenticated Playwright tests..."
pnpm --filter web exec playwright test services.spec.ts incidents.spec.ts settings.spec.ts
